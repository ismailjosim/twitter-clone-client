...
const SAMPLE_POSTS = [
    {
        ...
        content: (
            <>
                Before oil wealth, <span className="text-primary">Abu Dhabi</span> was isolated on the rim of the desert, a place no one wanted, sunk in poverty and isolated. Success can make history easy to forget.{" "}
    ...
    },
    {
        ...
        content: (
            <>
                Just shipped a new feature using <span className="text-primary">shadcn/ui</span> + <span className="text-primary">Tailwind CSS v4</span>. The DX is absolutely incredible. 🚀
            </>
        ),
        image: null,
            ...
            <div className="p-4 border-b border-border">
                <div className="flex gap-3">
                    <Avatar className="h-10 w-10">
                        ...
                        <AvatarFallback className="bg-muted text-muted-foreground text-xs font-semibold">MJ</AvatarFallback>
                    </Avatar>
                    <div className="flex-1">
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex gap-1 text-primary">
                                {[ImageIcon, FileText, BarChart3, Smile, Calendar].map((Icon, i) => (
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-1 min-w-0">
                                    <span className="font-bold text-sm hover:underline truncate">{post.name}</span>
                                    {post.verified && (
                                        <CheckCircle2 size={15} className="fill-primary text-background shrink-0" />
                                    )}
                            ...
                            <div className="flex justify-between mt-3 text-muted-foreground max-w-sm">
                                <button className="flex items-center gap-1 group">
                                    ...
                                    <span className="text-xs group-hover:text-primary transition">{post.stats.replies}</span>
                                </button>
                                <button className="flex items-center gap-1 group">
                                    <div className="p-2 group-hover:bg-green-500/10 group-hover:text-green-500 rounded-full transition">
                                        <Repeat2 size={17} />
                                    </div>
                                    <span className="text-xs group-hover:text-green-500 transition">{post.stats.reposts}</span>
                                </button>
                                <button
                                ...
                                >
                                    <div className={cn(
                                        ...
                                        <Heart size={17} className={cn(likedPosts.has(post.id) && "fill-pink-500")} />
                                    </div>
                                    <span className={cn(
                                        "text-xs transition",
                                        likedPosts.has(post.id) ? "text-pink-500" : "group-hover:text-pink-500"
                                    )}>
                                        {post.stats.likes}
                                    </span>
                                </button>
                                <button className="flex items-center gap-1 group">
                                    ...
                                    <span className="text-xs group-hover:text-primary transition">{post.stats.views}</span>
                                </button>
