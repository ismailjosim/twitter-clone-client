		...
		<aside className='h-screen sticky top-0 flex flex-col justify-between py-4 px-3 xl:px-4 w-18 xl:w-64 bg-sidebar border-r border-sidebar-border transition-all duration-200'>
				...
				{menu.map((item) => {
					...
					const NavButton = (
						...
						>
							...
							{item.badge && (
								<Badge className='ml-1 text-[10px] px-1.5 py-0 bg-primary text-primary-foreground rounded-sm font-bold'>
									{item.badge}
								</Badge>
							)}
						</Button>
			...
			<DropdownMenu>
				...
				>
					...
					<DropdownMenuItem className='cursor-pointer hover:bg-accent focus:bg-accent'></DropdownMenuItem>
				</DropdownMenuContent>
			</DropdownMenu>
		</aside>
	)
